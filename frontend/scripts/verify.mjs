...
await page.waitForSelector('text=Animation Composer')
console.log('1. page loaded, title visible: OK')

// Phase A: open Craft so param panel assertions still apply (Play is default)
...
const renderBtn = page.getByRole('button', { name: /Subscribe to render/ })
console.log('9. render CTA present and disabled:', await renderBtn.isDisabled())

// Share button present (clipboard may be empty until click; just assert enabled)
...
console.log(`10. randomize on strange-attractor: type=${typeAfterRandomize}, param a in [${randMin},${randMax}]=${randVal}: ${inRange}`)
...
console.log(`17. randomize changed a slider: ${defaultTheta} -> ${afterRandomize}`)

await page.getByRole('button', { name: /^↺ Reset/ }).click()
await waitUntil(async () => (await thetaSlider.inputValue()) === defaultTheta)
console.log('18. reset restored the default:', await thetaSlider.inputValue() === defaultTheta)

await page.getByRole('button', { name: /⧉ JSON|✓ Copied/ }).click()
